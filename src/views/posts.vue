<template>
  <div>
    <nvHead :title="title" :back="back"></nvHead>
    <div class="content">
      <mu-row gutter>
      ...
      <mu-raised-button v-show="postsMore" label="返回主页" :to="'/posts/'+ this.tag + '/1'" display="none" fullWidth/>
      <mu-raised-button v-show="postsLoad" label="点击加载下一页..." :to="'/posts/'+ this.tag + '/' + pageId" fullWidth/>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    getPosts() {
      this.http.get('https://sangsir.com/api/posts?pageSize=6&tag=' + this.tag + '&page=' + this.id)
        .then(res => {
          if(res.data.data == 0){
            this.postsMore = true
            ...
            this.title = this.postsInfo[0].categories[0].name
